...
function replaceMap() {
  ...
  for (i = MIN; i <= MAX; i++) {
    ...
    for (key in CONSTRUCTORS) {
      if (!CONSTRUCTORS.hasOwnProperty(key)) continue;
      replace(`\\b${key}`, `${CONSTRUCTORS[key]}["constructor"]`);
    }

    for (key in SIMPLE) {
      if (!SIMPLE.hasOwnProperty(key)) continue;
...
function replaceStrings() {
  ...
  function findMissing() {
    ...
    for (const _all in MAPPING) {
      if (!MAPPING.hasOwnProperty(_all)) continue;
      const _value = MAPPING[_all];

      if (_value && _value.match(regEx)) {
        missing[_all] = _value;
        done = true;
      }
    }
  ...
  while (findMissing()) {
    for (all in missing) {
      if (!missing.hasOwnProperty(all)) continue;
      value = MAPPING[all];
...
function encode(input, wrapWithEval, runInParentScope) {
  ...
  for (const i in SIMPLE) {
    if (!SIMPLE.hasOwnProperty(i)) continue;
    r += `${i}|`;
  }
  r += '.';
